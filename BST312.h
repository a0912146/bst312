...
int BST_312 <ItemType>::countNodes(TreeNode* t) const
{
    int Nodes=1;
    if(t->left) {
        Nodes = Nodes + countNodes(t->left);
    }
    if (t->right) {
        Nodes = Nodes + countNodes(t->right);
    }
    return Nodes;
...
bool BST_312 <ItemType>::isItemInTree(TreeNode* t, const ItemType& item) const
{
    while(t) {
        ...
        //go through the right subtree only
        else if (item > t->data) {
            t = t->right;
        }
        //go through the left subtree only
        else{
            t = t->left;
        }
    }
...
{
    if( isItemInTree(root, item) == true){
        return true;
    }
    else{
        return false;
